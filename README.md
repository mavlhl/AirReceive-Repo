...
| Component | Role |
|-----------|------|
| **Android app** (`app/`) | Runs on your phone, receives images, saves them to a local gallery, and can save to the device photo library. |
...
4. Create a file named `.env` in the project root and set `GEMINI_API_KEY` to your Gemini API key (see `.env.example`). This is used by AI Studio tooling if present; the core photo transfer does not require it.
5. Remove this line from `app/build.gradle.kts` if you are building locally without the project’s debug signing setup: `signingConfig = signingConfigs.getByName("debugConfig")`
